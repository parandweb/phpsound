<?php
// Language Name
$name = 'Română';

// Language Author
$author = 'phpSound';

// Language URL
$url = 'https://phpsound.com';

$LNG['lang_dir'] = 'ltr';

$LNG['user_success'] = 'Utilizator creeat cu succes';
$LNG['user_exists'] = 'Acest nume de utilizator există deja';
$LNG['email_exists'] = 'Acest email este folosit deja';
$LNG['all_fields'] = 'Toate câmpurile sunt obligatorii';
$LNG['user_alnum'] = 'Numele de utilizator trebuie să conțină doar litere și numere';
$LNG['user_too_short'] = 'Numele de utilizator trebuie să fie între 3 și 32 de caractere';
$LNG['user_limit'] = 'Prea multe conturi creeate de pe acest IP';
$LNG['invalid_email'] = 'Email invalid';
$LNG['activate_email'] = 'Un link de activare ți-a fost trimis pe email';
$LNG['account_activated'] = 'Contul tău a fost activat';
$LNG['invalid_user_pw'] = 'Utilizatorul sau parola sunt invalide';
$LNG['invalid_captcha'] = 'Captcha invalid';
$LNG['log_out'] = 'Deconectare';
$LNG['hello'] = 'Bun venit';
$LNG['connect'] = 'Conectare';
$LNG['register'] = 'Înregistrare';
$LNG['login'] = 'Autentificare';
$LNG['password'] = 'Parola';
$LNG['username'] = 'Utilizator';
$LNG['email'] = 'Email';
$LNG['captcha'] = 'Captcha';
$LNG['username_or_email'] = 'Utilizator sau email';
$LNG['welcome_desc'] = 'împarte-ți muzica, conectează-te cu artiștii, explorează noi melodii';
$LNG['join_now'] = 'Alătură-te';
$LNG['learn_more'] = 'Află mai multe';
$LNG['creators'] = 'Creatori';
$LNG['listeners'] = 'Ascultători';
$LNG['audience'] = 'Audiență';
$LNG['organize'] = 'Organizează';
$LNG['featured'] = 'Recomandat';
$LNG['popular'] = 'Populare';
$LNG['latest'] = 'Recente';
$LNG['upload_desc'] = 'Încarcă-ți sunetele, melodiile, podcast-urile pentru lume';
$LNG['audience_desc'] = 'Conectează-te cu comunitatea ta și crește-ți audiența';
$LNG['statistics_desc'] = 'Cunoaște-ți audiența melodiilor cu statistici avansate';
$LNG['explore_desc'] = 'Explorează și transmite muzică, conectează-te cu artiști noi';
$LNG['share_desc'] = 'Împărtășește-ți muzica favorită cu prietenii pe rețelele de socializare';
$LNG['organize_desc'] = 'Creează Playlist-uri și organizează-ți muzica cum îți place';
$LNG['forgot_password'] = 'Ai uitat parola?';
$LNG['all_rights_reserved'] = 'Toate drepturile rezervate';

// NOTIFICATION BOXES //
$LNG['settings_saved'] = 'Setări salvate';
$LNG['password_changed'] = 'Parola schimbată';
$LNG['nothing_changed'] = 'Nici o schimbare';
$LNG['password_not_changed'] = 'Parola n-a fost schimbată';
$LNG['wrong_current_password'] = 'Parola curentă pe care ai introdus-o este incorectă.';
$LNG['image_saved'] = 'Imagine salvată';
$LNG['profile_picture_saved'] = 'Imaginea de profil a fost schimbată';
$LNG['error'] = 'Eroare';
$LNG['no_file'] = 'Nu ai selectat nici un fișier pentru încărcare sau fișierul selectat este gol.';
$LNG['file_exceeded'] = 'Fișierul ales nu trebuie să depășească <strong>%s</strong> MB.';
$LNG['file_format'] = 'Formatul fișierului selectat nu este suportat. Încarcă în formatul <strong>%s</strong>';
$LNG['image_removed'] = 'Imagine ștersă';
$LNG['profile_picture_removed'] = 'Poza de profil a fost ștersă';
$LNG['profile_description'] = 'Descrierea profilului trebuie să fie de %s caractere sau mai puține.';
$LNG['playlist_description'] = 'Descrierea playlist-ului trebuie să fie de %s caractere sau mai puține.';
$LNG['playlist_name_empty'] = 'Numele playlist-ului nu poate să fi gol.';
$LNG['changes_saved'] = 'Modificările salvate cu succces.';
$LNG['valid_email'] = 'Introdu o adresă de email validă.';
$LNG['valid_url'] = 'Introdu o adresă web validă.';
$LNG['valid_country'] = 'Introdu un nume de țara valid';
$LNG['background_changed'] = 'Fundalul a fost salvat cu succes.';
$LNG['background_not_changed'] = 'Fundalul nu a putut fi schimbat.';
$LNG['password_too_short'] = 'Parola trebuie să conțină măcar <strong>6</strong> caractere.';
$LNG['password_not_match'] = 'Parola introdusă nu se potrivește.';
$LNG['username_not_found'] = 'Nu am putut identifica utilizatorul introdus.';
$LNG['userkey_not_found'] = 'Utilizatorul sau cheia de resetare sunt greșite, asigurați-vă că le-ați introdus corect.';
$LNG['email_reset'] = 'Un email conținând instrucțiuni de resetare a parolei au fost trimise. Te rugam așteaptă până la 24 de ore pentru ați trimite emailul, de asemenea verifică căsuța de Spam dacă nu-l găsești în Inbox.';
$LNG['user_deleted'] = 'Utilizator șters';
$LNG['user_has_been_deleted'] = 'Utilizatorul <strong>%s</strong> a fost șters.';
$LNG['user_not_deleted'] = 'Utilizatorul selectat (ID: %s) nu a putut fi șters.';
$LNG['user_not_exist'] = 'Utilizatorul selectat nu există';
$LNG['payment_not_exist'] = 'ID-ul plății selectate nu există.';
$LNG['report_not_exist'] = 'ID-ul raportului nu există.';
$LNG['theme_changed'] = 'Tema schimbată';
$LNG['notif_saved'] = 'Notificările salvate';
$LNG['notif_success_saved'] = 'Notificările salvate cu succes.';
$LNG['password_reset'] = 'Ți-ai resetat parola cu succes. Te poți autentifica cu noua parolă.';

// MAIL CONTENT //
$LNG['welcome_mail'] = 'Bine ai venit la %s';
$LNG['email_footer_unsub'] = '<br><br><span style="color: #aaa;">Acest mesaj a fost trimis automat, dacă nu doriți să mai primiți astfel de mesaje de la <strong>%s</strong> în viitor, vă rugam să vă <a href="%s">Dezabonați</a>.</span>';
$LNG['email_footer_gen'] = '<br><br><span style="color: #aaa;">Copyright &copy; '.date('Y').' <a href="%s">%s</a>. Toate drepturile rezervate.';
$LNG['email_hello'] = 'Bună ziua <strong>%s</strong>,<br><br>';
$LNG['user_created'] = 'Mulțumim că te-ai înregistrat pe <strong>%s</strong><br><br>Nume utilizator: <strong>%s</strong><br><br>Te poți autentifica la: <a href="%s" target="_blank">%s</a>'.$LNG['email_footer_gen'];
$LNG['recover_mail'] = 'Recuperare Parolă';
$LNG['recover_content'] = 'O cerere de recuperare a parolei a fost cerută, dacă nu tu ai făcut această cerere, te rugam să ignori acest email. <br><br>Nume utilizator: <strong>%s</strong><br>Cheie de resetare: <strong>%s</strong><br><br>Îți poți reseta parola accesând următorul link: <a href="%s" target="_blank">%s</a>'.$LNG['email_footer_gen'];
$LNG['ttl_comment_email'] = '%s ți-a comentat o melodie';
$LNG['comment_email'] = $LNG['email_hello'].'<strong><a href="%s">%s</a></strong> ți-a comentat <strong><a href="%s">melodia.</a></strong>'.$LNG['email_footer_unsub'];
$LNG['ttl_like_email'] = '%s ți-a apreciat melodia';
$LNG['like_email'] = $LNG['email_hello'].'<strong><a href="%s">%s</a></strong> ți-a apreciat <strong><a href="%s">melodia.</a></strong>'.$LNG['email_footer_unsub'];
$LNG['ttl_new_friend_email'] = '%s te-a adăugat ca prieten';
$LNG['new_friend_email'] = $LNG['email_hello'].'<strong><a href="%s">%s</a></strong> te-a adăugat ca prieten.'.$LNG['email_footer_unsub'];
$LNG['ttl_copyright_notification'] = 'Revendicare Copyright - %s';
$LNG['copyright_mail_0'] = $LNG['email_hello'].'Melodia <a href="%s">%s</a> ți-a fost suspendată în urma unei revendicări copyright de către <a href="%s">%s</a>. Dacă consideri că a fost o eroare, te rugăm să ne contactezi. ID caz: %s'.$LNG['email_footer_gen'];
$LNG['copyright_mail_1'] = $LNG['email_hello'].'Melodia violatoare <a href="%s">%s</a> a fost suspendată. Mulțumim. ID caz: %s'.$LNG['email_footer_gen'];
$LNG['ttl_suspended_account_mail'] = 'Contul dumneavoastră a fost suspendat';
$LNG['suspended_account_mail'] = $LNG['email_hello'].'Contul tău a fost suspendat. Dacă consideri ca a fost o eroare, te rugăm să ne contactezi.'.$LNG['email_footer_gen'];
$LNG['ttl_confirm_email'] = 'Activează-ți contul';
$LNG['confirm_email'] = $LNG['email_hello'].'Mulțumim că te-ai alăturat <strong>%s</strong>. Finalizați înregistrarea <a href="%s" target="_blank">dând click aici</a>.'.$LNG['email_footer_gen'];

// ADMIN PANEL //
$LNG['general_link'] = 'General';
$LNG['security_link'] = 'Securitate';
$LNG['manage_users'] = 'Administrează Utilizatori';

$LNG['theme_install'] = 'Pentru a instala o nouă temă, încarc-o în folderul <strong>themes</strong>';
$LNG['language_install'] = 'Pentru a instala o limbă, încărcați-o în folderul <strong>languages</strong>';
$LNG['theme_author_homepage'] = 'Vizitează pagina autorului';
$LNG['theme_version'] = 'Versiune';
$LNG['theme_active'] = 'Activă';
$LNG['theme_activate'] = 'Activează';
$LNG['by'] = 'De';

// STREAM //
$LNG['welcome_stream_ttl'] = 'Bine ai venit la Stream-ul tău.';
$LNG['welcome_stream'] = 'Toate melodiile prietenilor tăi v-or apărea pe această pagină, începe prin ați face noi prieteni.';
$LNG['leave_comment'] = 'Lasă un comentariu...';
$LNG['post'] = 'Postează';
$LNG['view_more_comments'] = 'Vezi mai multe comentarii';
$LNG['this_track_private'] = 'Această melodie este privată';
$LNG['this_track_public'] = 'Această melodie este publică';
$LNG['this_playlist_private'] = 'Acest playlist este privat';
$LNG['this_playlist_public'] = 'Acest playlist este public';
$LNG['delete_this_comment'] = 'Șterge acest comentariu';
$LNG['delete_this_message'] = 'Șterge acest mesaj';
$LNG['report_this_track'] = 'Raportează această melodie';
$LNG['report_this_comment'] = 'Raportează acest comentariu';
$LNG['load_more'] = 'Încarcă mai mult';
$LNG['view_more'] = 'Vezi mai mult';
$LNG['comment_wrong'] = 'Ceva nu a mers corect, te rugăm reîncarcă pagina și încearcă din nou.';
$LNG['comment_too_long'] = 'Ne pare rău, dar numărul maxim de caractere permis pe comentariu este <strong>%s</strong>.';
$LNG['comment_error'] = 'Ne pare rău, nu am putut publica acest comentariu, te rugăm reîncarcă pagina și încercă din nou.';
$LNG['track_hidden_1'] = 'Ne pare rău, dar această melodie este privată, doar autorul melodiei o poate vedea.';
$LNG['track_hidden_2'] = 'Ne pare rău, dar setările de profil ale acestui utilizator permit doar urmăritorilor lui să vadă această melodie.';
$LNG['track_hidden_1_ttl'] = $LNG['track_hidden_2_ttl'] = 'Melodie Privată';
$LNG['track_suspended_1'] = 'Ne pare rău, dar această melodie a fost dezactivată. Dacă consideri ca a fost o eroare, te rugăm să ne contactezi.';
$LNG['track_suspended_1_ttl'] = 'Melodie dezactivată';
$LNG['playlist_hidden'] = 'Ne pare rău, dar acest playlist este privat, doar autorul playlist-ului îl poate vedea.';
$LNG['playlist_hidden_ttl'] = 'Playlist Privat';
$LNG['comment'] = 'Comentează';
$LNG['share'] = 'Împarte';
$LNG['autoplay'] = 'Autoplay';
$LNG['add_to'] = 'Adaugă în';
$LNG['buy'] = 'Cumpără';
$LNG['download'] = 'Descarcă';
$LNG['new_playlist'] = 'Creează un nou playlist';
$LNG['cancel'] = 'Anulează';
$LNG['close'] = 'Închide';

// REPORT //
$LNG['1_not_exists'] = 'Melodia raportată nu există.';
$LNG['0_not_exists'] = 'Comentariul raportat nu există.';
$LNG['1_already_reported'] = 'Această melodie a fost deja raportată, și v-a fi analizat în cel mai scurt timp posibil, mulțumim.';
$LNG['0_already_reported'] = 'Acest comentariu a fost deja raportat, și v-a fi analizat în cel mai scurt timp posibil, mulțumim.';
$LNG['1_is_safe'] = 'Această melodie a fost marcată ca <strong>sigură</strong> de către un administrator, mulțumim de feedback.';
$LNG['0_is_safe'] = 'Acest comentariu a fost marcat ca <strong>sigur</strong> de către un administrator, mulțumim de feedback.';
$LNG['1_report_added'] = 'Melodia a fost raportată, mulțumim de feedback.';
$LNG['0_report_added'] = 'Comentariul a fost raportat, mulțumim de feedback.';
$LNG['1_report_error'] = 'Ne pare rău dar ceva nu a mers când raportam această melodie, te rugăm reîncarcă pagina și încercă din nou.';
$LNG['0_report_error'] = 'Ne pare rău dar ceva nu a mers când raportam acest comentariu, te rugăm reîncarcă pagina și încercă din nou.';
$LNG['1_is_deleted'] = 'Melodia a fost ștearsă, mulțumim de feedback.';
$LNG['0_is_deleted'] = 'Comentariul a fost șters, mulțumim de feedback.';
$LNG['deleted_track'] = 'Melodia a fost ștearsă.';
$LNG['deleted_comment'] = 'Comentariul a fost șters.';
$LNG['suspended_track'] = 'Melodia a fost suspendată.';
$LNG['restored_track'] = 'Melodia a fost restaurată.';
$LNG['safe_report'] = 'Raportul a fost marcat ca sigur.';
$LNG['reason'] = 'Motiv';
$LNG['report_ci'] = 'Încălcare drepturi autor';
$LNG['report_track'] = 'Raportează melodie';
$LNG['signature'] = 'Semnatură';
$LNG['sub_signature'] = 'Scriind numele tău complet în această căsuță, va fi considerat ca fiind semnatura ta electronică.';
$LNG['report_description'] = 'Te rugăm identifică și descrie cât mai detaliat posibil lucrarea cu drepturi de autor care consideri că sunt încălcate.';
$LNG['report1'] = 'Am bună credință și declar că lucrarea cu drepturile de autor descrise mai sus nu este autorizată de către deținătorul drepturilor de autor, agentul lui sau lege.';
$LNG['report2'] = 'Confirm că informațiile din această notificare sunt corecte';
$LNG['report3'] = 'Confirm că sunt deținătorul drepturilor de autor sau că sunt autorizat să actionez în numele deținătorului drepturilor de autor ale cărui drepturi îi sunt încălcate. Fac apel la legea United States Digital Millennium Copyright Act U.S.C. 17 &sect;512(c), fac această declarație sub pedeapsă de sperjur.';
$LNG['rep_resc_error'] = 'Descrierea trebuie să fie mai mică de %s de caractere.';
$LNG['abusive_comment'] = 'Comentariu abuziv';
$LNG['abusive_track'] = 'Melodie abuzivă';
$LNG['copyright_infringement'] = 'Încalcare drepturi de autor';
$LNG['claimant'] = 'Reclamant';
$LNG['infringing_material'] = 'Materialul în cauză';
$LNG['reported_by'] = 'Raportat de';
$LNG['delete_track'] = 'Șterge Melodia';
$LNG['delete_comment'] = 'Șterge Comentariul';
$LNG['delete_report'] = 'Șterge Raportul';
$LNG['suspend_track'] = 'Suspendă Melodia';
$LNG['restore_track'] = 'Restaurează Melodia';
$LNG['view_comment'] = 'Vezi comentariu';

// SIDEBAR //
$LNG['filter_notifications'] = 'Filtrează Notificările';
$LNG['archive'] = 'Arhive';
$LNG['statistics'] = 'Statistici';
$LNG['all_notifications'] = 'Toate notificările';
$LNG['all_time'] = 'Tot timpul';
$LNG['subscriptions'] = 'Urmăresc';
$LNG['subscribers'] = 'Urmăritori';
$LNG['welcome'] = 'Bine ai venit';
$LNG['sidebar_people'] = 'Persoane';
$LNG['sidebar_tracks'] = 'Melodii';
$LNG['sidebar_playlists'] = 'Playlist-uri';
$LNG['online_friends'] = 'Prieteni online';
$LNG['sidebar_likes'] = 'Aprecieri';
$LNG['sidebar_comments'] = 'Comentarii';
$LNG['sidebar_friendships'] = 'Prietenii';
$LNG['sidebar_chats'] = 'Discuții';
$LNG['sidebar_suggestions'] = 'Sugestii de prietenii';
$LNG['sidebar_trending'] = 'Subiecte in trend';
$LNG['sidebar_friends_activity'] = 'Activitatea prietenilor';
$LNG['edit_track'] = 'Editează melodie';
$LNG['edit_playlist'] = 'Editează playlist';
$LNG['go_back'] = 'Înapoi';

// MESSAGES / CHAT //
$LNG['lonely_here'] = 'Este gol aici, ce zici de niște prieteni noi?';
$LNG['write_message'] = 'Scrie un mesaj...';
$LNG['chat_too_long'] = 'Ne pare rău dar numărul maxim de caractere admise este de <strong>%s</strong>.';
$LNG['blocked_by'] = 'Acest mesaj nu s-a putut trimite. <strong>%s</strong> v-a blocat.';
$LNG['blocked_user'] = 'Acest mesaj nu s-a putut trimite. L-ai blocat pe <strong>%s</strong>.';
$LNG['chat_self'] = 'Ne pare rău dar nu poți trimite un mesaj către tine.';
$LNG['chat_no_user'] = 'Trebuie să alegeți un utilizator cu care să conversați.';
$LNG['view_more_conversations'] = 'Încarcă mai mult din conversație';
$LNG['block'] = 'Blochează';
$LNG['unblock'] = 'Deblochează';
$LNG['conversation'] = 'Conversație';
$LNG['start_conversation'] = 'Poți începe o conversație alegând o persoană din lista ta de prieteni.';
$LNG['send_message'] = 'Trimite mesaj';

// UPLOAD / EDIT //
$LNG['upload_track'] = 'Încarcă Melodie';
$LNG['upload_image'] = 'Încarcă Imagine';
$LNG['track_selected'] = 'Melodie selectată';
$LNG['image_selected'] = 'Imagine selectată';

$LNG['0_upload_err'] = 'Contul tău a depășit limita totală de înărcare permisă. Melodia <strong>%s</strong> nu a fost încărcată';
$LNG['1_upload_err'] = 'Nici o melodie selectată';
$LNG['2_upload_err'] = 'Melodia <strong>%s</strong> este prea mare. Mărimea maximă permisă este de %s';
$LNG['3_upload_err'] = 'Melodia <strong>%s</strong> nu are un format valid. Încarcă fișier audio de tip %s';
$LNG['4_upload_err'] = 'Coperta selectată este prea mare. Mărimea maximă permisă este de %s';
$LNG['5_upload_err'] = 'Coperta selectată nu are un format avlid. Încarcă fișier de tip %s';
$LNG['6_upload_err'] = 'Descrierea trebuie să fie mai scurtă de %s de caractere';
$LNG['7_upload_err'] = 'Link-ul de cumpărare este invalid';
$LNG['8_upload_err'] = 'Poți adăuga maxim %s tag-uri';
$LNG['9_upload_err'] = 'Trebuie să adaugi măcar %s tag(uri)';
$LNG['10_upload_err'] = 'Titlul melodiei nu poate fi gol';
$LNG['11_upload_err'] = 'Titlul melodiei trebuie să fie mai mic de %s caractere';
$LNG['12_upload_err'] = 'Numele unui tag trebuie să fie mai mic de %s caractere';
$LNG['track_updated'] = 'Melodia a fost actualizată cu succes';
$LNG['view_track'] = 'Vezi Melodie';
$LNG['congratulations'] = 'Felicitări';
$LNG['track_uploaded'] = 'Melodia <strong><a href="%s" rel="loadpage">%s</a></strong> a fost încărcată';
$LNG['playlist_created'] = 'Playlist-ul <strong><a href="%s" rel="loadpage">%s</a></strong> a fost creeat';
$LNG['or'] = 'sau';

$LNG['license'] = 'Licență';
$LNG['licensed_under'] = 'Licențiată sub';
$LNG['metadata'] = 'Metadata';
$LNG['permissions'] = 'Permisii';
$LNG['buy_link'] = 'Link cumpărare';
$LNG['release_date'] = 'Data lansării';
$LNG['tags'] = 'Tag-uri';
$LNG['edit_tags'] = 'separate de virgulă, incluzând gen-uri muzicale';
$LNG['visibility'] = 'Vizibilitate';
$LNG['allow_downloads'] = 'Permite descărcarea';
$LNG['record_label'] = 'Casa de discuri';
$LNG['all_rights'] = 'Toate drepturile rezervate';
$LNG['creative_commons'] = 'Creative Commons';
$LNG['license_at'] = 'Atribuire';
$LNG['license_nc'] = 'Necomercial';
$LNG['license_nd'] = 'Fară derivate';
$LNG['license_sa'] = 'Distribuire intactă';
$LNG['license_at_desc'] = 'Permiți altora să copieze, distribuie, afișeze și interpreta lucrarea ta - și să facă derivări ale lucrării tale - doar dacă îți acordă creditul pe care îl ceri.';
$LNG['license_nc_desc'] = 'Permiți altora să copieze, distribuie, afișeze și interpreta lucrarea ta - și să facă derivări ale lucrării tale - doar pentru scopuri necomerciale.';
$LNG['license_nd_desc'] = 'Permiți altora să copieze, distribuie, afișeze și interpreta lucrarea ta în original, fară lucrări derivate ale lucrării tale.';
$LNG['license_sa_desc'] = 'Permiți altora să distribuie lucrări derivate doar sub licență identică cu cea care guvernează lucrarea ta.';

// USER PANEL //
$LNG['user_menu_general'] = 'General';
$LNG['user_menu_security'] = 'Parolă';
$LNG['user_menu_avatar'] = 'Profil';
$LNG['user_menu_notifications'] = 'Notificări';
$LNG['user_menu_social'] = 'Social';
$LNG['user_menu_plan'] = 'Plan Cont';
$LNG['user_menu_blocked'] = $LNG['user_ttl_blocked'] = 'Utilizatori Blocați';
$LNG['user_menu_delete'] = $LNG['user_ttl_delete'] = 'Șterge Cont';

$LNG['user_ttl_general'] = 'Setări Generale';
$LNG['user_ttl_security'] = 'Setări Parolă';
$LNG['user_ttl_avatar'] = 'Setări Profil';
$LNG['user_ttl_notifications'] = 'Setări Notificări';
$LNG['user_ttl_social'] = 'Profile rețele sociale';
$LNG['user_ttl_plan'] = 'Plan Cont';

$LNG['user_desc_general'] = 'Schimbă-ți setări de cont, intimitate și locație.';
$LNG['user_desc_security'] = 'Schimbă-ți parola.';
$LNG['user_desc_avatar'] = 'Schimbă-ți imaginea de profil';
$LNG['user_desc_cover'] = 'Schimbă-ți imaginea de copertă';
$LNG['user_desc_notifications'] = 'Schimbă-ți setările de notificări.';
$LNG['blocked_desc'] = 'Persoanele blocate nu te mai pot contacta prin mesaje.';
$LNG['delete_acc_desc'] = 'Contul tău va fi șters definitiv, împreună cu tot conținutul asociat acestuia.';

$LNG['ttl_background'] = 'Fundaluri';
$LNG['sub_background'] = 'Alege un fundal pentru profilul tău';

$LNG['ttl_first_name'] = 'Nume';
$LNG['sub_first_name'] = 'Introduceți numele de familie';

$LNG['ttl_last_name'] = 'Prenume';
$LNG['sub_last_name'] = 'Introduceți prenumele';

$LNG['ttl_email'] = 'Email';
$LNG['sub_email'] = 'Adresa email nu va fi afișată';

$LNG['ttl_country'] = 'Țara';
$LNG['sub_country'] = 'Țara în care locuiești';

$LNG['ttl_city'] = 'Oraș';
$LNG['sub_city'] = 'Orașul în care locuiești';

$LNG['ttl_website'] = 'Website';
$LNG['sub_website'] = 'Dacă aveți un blog, o pagina personala, un site, introduceți-l';

$LNG['ttl_profile'] = 'Profil';
$LNG['sub_profile'] = 'Vizibilitate profil';

$LNG['ttl_offline'] = 'Vizibilitate Chat';
$LNG['sub_offline'] = 'Starea vizibilitații în chat';

$LNG['ttl_facebook'] = 'Facebook';
$LNG['sub_facebook'] = 'ID-ul d-voastră de Facebook';

$LNG['ttl_twitter'] = 'Twitter';
$LNG['sub_twitter'] = 'ID-ul d-voastră de Twitter';

$LNG['ttl_instagram'] = 'Instagram';
$LNG['sub_instagram'] = 'ID-ul d-voastră de Instagram';

$LNG['ttl_youtube'] = 'YouTube';
$LNG['sub_youtube'] = 'ID-ul d-voastră de YouTube';

$LNG['ttl_soundcloud'] = 'SoundCloud';
$LNG['sub_soundcloud'] = 'ID-ul d-voastră de SoundCloud';

$LNG['ttl_myspace'] = 'MySpace';
$LNG['sub_myspace'] = 'ID-ul d-voastră de MySpace';

$LNG['ttl_tumblr'] = 'Tumblr';
$LNG['sub_tumblr'] = 'ID-ul d-voastră de Tumblr';

$LNG['ttl_vimeo'] = 'Vimeo';
$LNG['sub_vimeo'] = 'ID-ul d-voastră de Vimeo';

$LNG['ttl_lastfm'] = 'Last.fm';
$LNG['sub_lastfm'] = 'ID-ul d-voastră de Last.fm';

$LNG['ttl_description'] = 'Descriere';
$LNG['sub_description'] = 'Descriere profil (160 de caractere sau mai puține)';

$LNG['ttl_upload_avatar'] = 'Încarcă imaginea de profil selectată';
$LNG['ttl_delete_avatar'] = 'Șterge imaginea de profil curentă';

$LNG['opt_public'] = 'Public';
$LNG['opt_private'] = 'Privat';
$LNG['opt_semi_private'] = 'Doar prienii';

$LNG['opt_offline_off'] = 'Online (când sunteți disponibil)';
$LNG['opt_offline_on'] = 'Offline tot timpul';

$LNG['ttl_new_password'] = 'Noua Parolă';
$LNG['sub_new_password'] = 'Introdu o parolă nouă (cel puțin 6 caractere).';
$LNG['ttl_repeat_password'] = 'Repetă Parola';
$LNG['sub_repeat_password'] = 'Reintrodu noua parolă';
$LNG['ttl_current_password'] = 'Parola Actuală';
$LNG['sub_current_password'] = 'Introdu parola actuală';
$LNG['save_changes'] = 'Salvează Modificările';
$LNG['ttl_upload_photo'] = 'Încarcă Imagine';
$LNG['ttl_upload_cover'] = 'Încarcă Copertă';
$LNG['ttl_delete_photo'] = 'Șterge Imagine';

$LNG['ttl_newslettern'] = 'Buletin informativ';
$LNG['sub_newslettern'] = 'Primește emailuri cu buletine informative';

$LNG['ttl_notificationl'] = 'Notificări Aprecieri';
$LNG['sub_notificationl'] = 'Afișează alertă și notificare pentru <strong>Aprecieri</strong>';

$LNG['ttl_notificationc'] = 'Notificări Comentarii';
$LNG['sub_notificationc'] = 'Afișează alertă și notificare pentru <strong>Comentarii</strong>';

$LNG['ttl_notificationd'] = 'Notificări Conversații';
$LNG['sub_notificationd'] = 'Afișează alertă și notificare pentru <strong>Conversații</strong>';

$LNG['ttl_notificationf'] = 'Notificări Prietenii Noi';
$LNG['sub_notificationf'] = 'Afișează alertă și notificare pentru <strong>Prietenii Confirmate</strong>';

$LNG['ttl_email_comment'] = 'Email la Comentarii';
$LNG['sub_email_comment'] = 'Primește email când cineva comentează la mesajul tău';

$LNG['ttl_email_like'] = 'Email la Aprecieri';
$LNG['sub_email_like'] = 'Primește email când cineva apreciază un mesaj de-al tău';

$LNG['ttl_email_new_friend'] = 'Email la Prietenii';
$LNG['sub_email_new_friend'] = 'Primește email când cineva îți trimite sau confirmă o cerere de prietenie';

$LNG['user_ttl_sidebar'] = 'Setări';

// ADMIN PANEL //
$LNG['admin_login'] = 'Autentificare Admin';
$LNG['admin_user_name'] = 'Utilizator';
$LNG['desc_admin_user'] = 'Introduceți numele de Administrator';
$LNG['admin_pass'] = 'Parola';
$LNG['desc_admin_pass'] = 'Introduceți parola de Administrator';
$LNG['admin_menu_logout'] = 'Deconectare';
$LNG['admin_ttl_sidebar'] = 'Meniu';
$LNG['admin_menu_dashboard']			= $LNG['admin_ttl_dashboard']		= 'Panou Control';
$LNG['admin_menu_site_settings']		= $LNG['admin_ttl_site_settings']	= 'Setări Site';
$LNG['admin_menu_security']				= $LNG['admin_ttl_security']		= 'Parolă';
$LNG['admin_menu_manage_users'] 		= $LNG['admin_ttl_users']			= 'Manageriază Utilizatori';
$LNG['admin_menu_stats'] 				= $LNG['admin_ttl_stats'] 			= 'Statistici';
$LNG['admin_menu_users_settings'] 		= $LNG['admin_ttl_users_settings']	= 'Setări Utilizatori';
$LNG['admin_menu_social'] 				= $LNG['admin_ttl_social']			= 'Autentificare Socială';
$LNG['admin_menu_pro'] 					= $LNG['admin_ttl_pro']				= 'Conturi Pro';
$LNG['admin_menu_themes'] 				= $LNG['admin_ttl_themes']			= 'Teme';
$LNG['admin_menu_languages']			= $LNG['admin_ttl_languages'] 		= 'Limbi';
$LNG['admin_menu_manage_payments']		= $LNG['admin_ttl_payments']		= 'Manageriază Plăți';
$LNG['admin_menu_manage_reports']		= $LNG['admin_ttl_reports']			= 'Manageriază Raportări';
$LNG['admin_menu_manage_ads'] 			= $LNG['admin_ttl_ads']				= 'Manageriază Reclame';
$LNG['admin_menu_manage_categories']	= $LNG['admin_ttl_categories']		= 'Manageriază Categorii';
$LNG['admin_menu_info_pages']			= $LNG['admin_ttl_info_pages'] 		= 'Info Pagini';
$LNG['admin_menu_newsletter']			= $LNG['admin_ttl_newsletter'] 		= 'Buletin Informativ';

$LNG['admin_ttl_title'] = 'Titlu';
$LNG['admin_sub_title'] = 'Titlul site-ului';

$LNG['logo'] = 'Siglă';
$LNG['admin_sub_logo'] = 'Sigla site-ului (format PNG)';

$LNG['admin_ttl_captcha'] = 'Captcha';
$LNG['admin_sub_captcha'] = 'Activează Captcha la înregistrare';

$LNG['admin_ttl_timestamp'] = 'Timestamp';
$LNG['admin_sub_timestamp'] = 'Tipul timestamp-ului pentru Mesaje, Comentarii și Conversații';

$LNG['admin_ttl_timezone'] = 'Fus orar';
$LNG['admin_sub_timezone'] = 'Fus orar suportat de PHP';

$LNG['admin_ttl_tracks_perpage'] = 'Melodii';
$LNG['admin_sub_tracks_perpage'] = 'Numărul melodiilor pe pagină';

$LNG['admin_ttl_com_perpage'] = 'Comentarii';
$LNG['admin_sub_com_perpage'] = 'Numărul comentariilor pe pagină';

$LNG['admin_ttl_chat_perpage'] = 'Conversații';
$LNG['admin_sub_chat_perpage'] = 'Numărul conversațiilor pe pagină';

$LNG['admin_ttl_permalinks'] = 'Permalinkuri';
$LNG['admin_sub_permalinks'] = 'Activează structură de link-uri permanente (example.com/profile/username)';

$LNG['admin_ttl_email_activation'] = 'Activare Email';
$LNG['admin_sub_email_activation'] = 'Necesită activarea contului prin email';

$LNG['admin_ttl_volume'] = 'Volum';
$LNG['admin_sub_volume'] = 'Setează volumul predefinit';

$LNG['admin_ttl_nperpage'] = 'Notificări';
$LNG['admin_sub_nperpage'] = 'Numărul de notificări afișate (Pagina de Notificări)';

$LNG['admin_ttl_message_limit'] = 'Limită Mesaje';
$LNG['admin_sub_message_limit'] = 'Numărul de caractere permise pe comentariu și discuții în chat';

$LNG['admin_ttl_email_reg'] = 'Email la Înregistrare';
$LNG['admin_sub_email_reg'] = 'Trimite email la înregistrarea utilizatorilor';

$LNG['admin_ttl_notificationsn'] = 'Notificări Evenimente';
$LNG['admin_sub_notificationsn'] = 'Intervalul de timp pentru a verifica Notificări noi (în secunde)';

$LNG['admin_ttl_chatrefresh'] = 'Actualizare Chat';
$LNG['admin_sub_chatrefresh'] = 'Cât de des se actualizează fereastra pentru a afișa conversațiile noi (în secunde)';

$LNG['admin_ttl_timeonline'] = 'Utilizatori Online';
$LNG['admin_sub_timeonline'] = 'Timpul cât un utilizator este considerat online de la ultima sa activate (în secunde)';

$LNG['admin_ttl_image_profile'] = 'Mărime Imagine (Profil)';
$LNG['admin_sub_image_profile'] = 'Mărimea imaginii admise pentru încărcare (imagine și copertă profil) (în MB)';

$LNG['admin_ttl_image_format'] = 'Format Imagine (Profil)';
$LNG['admin_sub_image_format'] = 'Format imagine admis pentru încărcare (avatar și copertă profil), folosiți doar gif,png,jpg alte formate nu sunt suportate';

$LNG['admin_ttl_cover_size'] = 'Mărime Copertă Album';
$LNG['admin_sub_cover_size'] = 'Dimensiunea imaginilor permise pentru încărcarea copertelor de album (în MB)';

$LNG['admin_ttl_cover_format'] = 'Format Copertă Album';
$LNG['admin_sub_cover_format'] = 'Format imagine admis pentru încărcarea copertelor de album (folosiți doar gif,png,jpg,jpeg alte formate nu sunt suportate)';

$LNG['admin_ttl_track_size'] = 'Dimensiune Melodie';
$LNG['admin_sub_track_size'] = 'Dimensiune admisa pentru încărcare de utilizatori (în MB)';

$LNG['admin_ttl_track_format'] = 'Format Melodie';
$LNG['admin_sub_track_format'] = 'Format melodie admis pentru încărcare, folosiți doar mp3,m4a,mp4 alte formate nu sunt suportate';

$LNG['admin_ttl_total_upload_size'] = 'Încărcare Total';
$LNG['admin_sub_total_upload_size'] = 'Încărcare totala admisă de melodii pentru un utilizator (în MB)';

$LNG['admin_ttl_pro_track_size'] = 'Dimensiune Melodie Pro';
$LNG['admin_sub_pro_track_size'] = 'Dimensiune admisă pentru încărcare de utilizatori pro (în MB)';

$LNG['admin_ttl_pro_total_size'] = 'Încărcare Total Pro';
$LNG['admin_sub_pro_total_size'] = 'Încărcare totala admisă de melodii pentru un utilizator pro (în MB)';

$LNG['admin_as3'] = 'Stocare Amazon S3';
$LNG['admin_sub_as3'] = 'Activează stocarea cu Amazon S3';

$LNG['admin_as3_key'] = 'Key';
$LNG['admin_sub_as3_key'] = 'Amazon S3 Access Key ID';

$LNG['admin_as3_secret'] = 'Secret Key';
$LNG['admin_sub_as3_secret'] = 'Amazon S3 Secret Access Key';

$LNG['admin_as3_region'] = 'Region';
$LNG['admin_sub_as3_region'] = 'Amazon S3 Region';

$LNG['admin_as3_bucket'] = 'Bucket';
$LNG['admin_sub_as3_bucket'] = 'Amazon S3 Bucket Name';

$LNG['admin_ttl_fbapp'] = 'Autentificare Facebook';
$LNG['admin_sub_fbapp'] = 'Permite utilizatorilor autentificarea prin Facebook';

$LNG['admin_ttl_fbappid'] = 'App ID';
$LNG['admin_sub_fbappid'] = 'Facebook App ID';

$LNG['admin_ttl_fbappsecret'] = 'App Secret'; 
$LNG['admin_sub_fbappsecret'] = 'Facebook App Secret';

$LNG['smtp_emails'] = 'Emailuri SMTP';
$LNG['smtp_sub_emails'] = 'Activează trimiterea de emailuri prin SMTP';

$LNG['smtp_host'] = 'Host SMTP';
$LNG['smtp_sub_host'] = 'Hostul SMTP';

$LNG['smtp_port'] = 'Port SMTP';
$LNG['smtp_sub_port'] = 'Portul SMTP';

$LNG['smtp_secure'] = 'SMTP Secure';
$LNG['smtp_sub_secure'] = 'Activează SMTP Secure';

$LNG['smtp_auth'] = 'Autentificare SMTP';
$LNG['smtp_sub_auth'] = 'Activează Autentificare SMTP';

$LNG['smtp_username'] = 'Utilizator SMTP';
$LNG['smtp_sub_username'] = 'Utilizatorul SMTP';

$LNG['smtp_password'] = 'Parolă SMTP';
$LNG['smtp_sub_password'] = 'Parola SMTP';

$LNG['standard_accounts'] = 'Conturi Standard';
$LNG['admin_ttl_ppapp'] = 'Conturi Pro';
$LNG['admin_sub_ppapp'] = 'Activează Conturi Pro prin PayPal';

$LNG['admin_ttl_ppclientid'] = 'PayPal Client ID';
$LNG['admin_sub_ppclientid'] = 'PayPal Client ID';

$LNG['admin_ttl_ppsecret'] = 'PayPal Secret';
$LNG['admin_sub_ppsecret'] = 'PayPal Secret';

$LNG['admin_ttl_ppsand'] = 'PayPal Sandbox';
$LNG['admin_sub_ppsand'] = 'PayPal sandbox pentru modul dezvoltator';

$LNG['admin_ttl_pro_plan_month'] = 'Plan Lunar';
$LNG['admin_sub_pro_plan_month'] = 'Valoarea planului Pro lunar (ex: 3.00 pentru 3 USD)';

$LNG['admin_ttl_pro_plan_year'] = 'Plan Anual';
$LNG['admin_sub_pro_plan_year'] = 'Valoarea planului Pro anual (ex: 29.00 pentru 29 USD)';

$LNG['admin_ttl_currency'] = 'Valută';
$LNG['admin_sub_currency'] = 'Tipul valutei pentru planuri';

$LNG['admin_ttl_ad1'] = 'Unitate reclamă 1';
$LNG['admin_sub_ad1'] = 'Unitatea 1 de reclamă (Subsol [Pagină Welcome])';

$LNG['admin_ttl_ad2'] = 'Unitate reclamă 2';
$LNG['admin_sub_ad2'] = 'Unitatea 2 de reclamă (Bară Laterală [Pagină Exploră])';

$LNG['admin_ttl_ad3'] = 'Unitate reclamă 3';
$LNG['admin_sub_ad3'] = 'Unitatea 3 de reclamă (Bară Laterală [Pagină Stream])';

$LNG['admin_ttl_ad4'] = 'Unitate reclamă 4';
$LNG['admin_sub_ad4'] = 'Unitatea 4 de reclamă (Bară Laterală [Pagină Profil])';

$LNG['admin_ttl_ad5'] = 'Unitate reclamă 5';
$LNG['admin_sub_ad5'] = 'Unitatea 5 de reclamă (Bară Laterală [Pagină Melodie și Playlist])';

$LNG['admin_ttl_ad6'] = 'Unitate reclamă 6';
$LNG['admin_sub_ad6'] = 'Unitatea 6 de reclamă (Bară Laterală [Pagină de căutare])';

$LNG['admin_ttl_edit_profile'] = 'Editează Profil';

$LNG['admin_ttl_delete_profile'] = 'Șterge Profil';

$LNG['admin_ttl_mail'] = 'Email';
$LNG['admin_ttl_username'] = 'Utilizator';
$LNG['admin_ttl_id'] = 'ID'; // As in user ID

$LNG['admin_ttl_notificationl'] = 'Notificări Aprecieri';
$LNG['admin_sub_notificationl'] = 'Afișează alertă și notificare pentru <strong>Aprecieri</strong> (utilizatorul poate schimba această opțiune)';

$LNG['admin_ttl_notificationc'] = 'Notificări Comentarii';
$LNG['admin_sub_notificationc'] = 'Afișează alertă și notificare pentru <strong>Comentarii</strong> (utilizatorul poate schimba această opțiune)';

$LNG['admin_ttl_notificationd'] = 'Notificări Conversații';
$LNG['admin_sub_notificationd'] = 'Afișează alertă și notificare pentru <strong>Conversații</strong> (utilizatorul poate schimba această opțiune)';

$LNG['admin_ttl_notificationf'] = 'Notificări Prietenii';
$LNG['admin_sub_notificationf'] = 'Afișează alertă și notificare pentru <strong>Prietenii Adăugate</strong> (utilizatorul poate schimba această opțiune)';

$LNG['admin_ttl_email_comment'] = 'Email la Comentarii';
$LNG['admin_sub_email_comment'] = 'Permite trimiterea de emailuri când cineva comentează la un mesaj';

$LNG['admin_ttl_email_like'] = 'Email la Aprecieri';
$LNG['admin_sub_email_like'] = 'Permite trimiterea de emailuri când cineva comentează apreciază un mesaj';

$LNG['admin_ttl_email_new_friend'] = 'Email la Prietenii Noi';
$LNG['admin_sub_email_new_friend'] = 'Permite trimiterea de emailuri când cineva adaugă un prieten nou';

$LNG['admin_ttl_wholiked'] = 'Cine Apreciază';
$LNG['admin_sub_wholiked'] = 'Numărul maxim de avatare afișat lângă numărul de aprecieri';

$LNG['admin_ttl_rperpage'] = 'Report-uri';
$LNG['admin_sub_rperpage'] = 'Numărul de report-uri afișate pe pagină (Manageriază Raportări)';

$LNG['admin_ttl_sperpage'] = 'Prieteni';
$LNG['admin_sub_sperpage'] = 'Numărul de prieteni afișat pe pagină (pagina de profil)';

$LNG['admin_ttl_aperip'] = 'Conturi';
$LNG['admin_sub_aperip'] = 'Numărul de conturi permise pentru înregistrare pe IP (0 pentru nelimitat)';

$LNG['admin_ttl_ronline'] = 'Prieteni Online';
$LNG['admin_sub_ronline'] = 'Numărul de prieteni online afișați pe pagina de Feed/Abonamente (Bară laterală).';

$LNG['admin_ttl_nperwidget'] = 'Notificări Dropdown';
$LNG['admin_sub_nperwidget'] = 'Numărul de notificări afișate pe categorii (Aprecieri, Comentarii, Mesaje, Împărtășiri, Cereri de prietenie)';

$LNG['admin_sub_tracking'] = 'Codul de monitorizare analitică';

$LNG['page_title'] = 'Titlu pagină';
$LNG['page_url'] = 'URL pagină';
$LNG['public_page'] = 'Pagină Publică';
$LNG['page_content'] = 'Conținut Pagină';
$LNG['create_page'] = 'Creează o pagină';
$LNG['email_title'] = 'Titlu Email';
$LNG['email_content'] = 'Conținut Email';

$LNG['sub_page_title'] = 'Titlul paginii (suportă texte translatabile)';
$LNG['sub_page_url'] = 'URL-ul paginii (fără caractere speciale)';
$LNG['sub_public_page'] = 'Afisează pagina în subsol și meniul lateral';
$LNG['sub_page_content'] = 'Conținutul paginii (suportă texte translatabile)';
$LNG['url_exists'] = 'Acest URL de pagină există deja';
$LNG['page_deleted'] = 'Pagina <strong>%s</strong> a fost ștearsă';
$LNG['newsletter_deleted'] = 'Buletinul informativ <strong>%s</strong> a fost șters';
$LNG['newsletter_sent'] = 'Buletinul informativ trimis';

$LNG['per_page'] = '/ pagină';
$LNG['per_ip'] = '/ IP';
$LNG['second'] = 'secundă';
$LNG['seconds'] = 'secunde';
$LNG['minute'] = 'minut';
$LNG['minutes'] = 'minute';
$LNG['hour'] = 'oră';
$LNG['recommended'] = 'recomandat';
$LNG['edit_user'] = 'Editează User';
$LNG['username_to_edit'] = 'Nume utilizator';
$LNG['username_to_edit_sub'] = 'Introduceți un nume de utilizator pentru a fi editat';
$LNG['add_category'] = 'Adaugă categorie';
$LNG['add_category_desc'] = 'Aceste categorii apar pe pagina Exploră';
$LNG['case_id'] = 'ID Caz';
$LNG['enter_case_id'] = 'Introdu ID-ul de caz pe care dorești să-l vezi';
$LNG['valid_rcs'] = 'Raporturi valide';
$LNG['payment_id'] = 'ID Plantă';
$LNG['enter_payment_id'] = 'Introdu ID-ul plășii pe care doreși s-o vezi';

// STATS //
$LNG['user_registration'] = 'Utilizatori Înregistrați';
$LNG['users_today'] = 'Astăzi';
$LNG['users_this_month'] = 'Luna aceasta';
$LNG['users_last_30'] = 'Ultimele 30 de zile';
$LNG['total_users'] = 'Total';
$LNG['plays_today'] = 'Redări Astăzi';
$LNG['plays_yesterday'] = 'Redări Ieri';
$LNG['plays_total'] = 'Redări Totale';
$LNG['total_plays'] = 'Redări Totale';
$LNG['plays_this_month'] = 'Redări luna aceasta';
$LNG['tracks_uploaded'] = 'Melodii Încărcate';
$LNG['used_space'] = 'Spațiu Folosit';
$LNG['free_space'] = 'Spațiu Liber';
$LNG['filter_stats'] = 'Filtrează Statistici';
$LNG['stats_today'] = 'Astăzi';
$LNG['stats_last7'] = 'Ultimele 7 zile';
$LNG['stats_last30'] = 'Ultimele 30 zile';
$LNG['stats_last356'] = 'Ultimele 12 luni';
$LNG['stats_total'] = 'Total';
$LNG['most_played'] = 'Cele mai redate';
$LNG['most_liked'] = 'Cele mai apreciate';
$LNG['most_commented'] = 'Cele mai comentate';
$LNG['who_played_most'] = 'Cine a redat cel mai mult';
$LNG['who_downloaded_most'] = 'Cine a descărcat cel mai mult';
$LNG['top_countries'] = 'Top țări';
$LNG['top_cities'] = 'Top orașe';
$LNG['stats_note'] = 'Notă: Statisticile sunt generate de la utilizatorii înregistrate.';
$LNG['users'] = 'Utilizatori';
$LNG['geographic'] = 'Geografic';

$LNG['plays'] = 'Redări';
$LNG['comments'] = 'Comentarii';
$LNG['downloads'] = 'Descărcări';
$LNG['tracks_and_comments'] = 'Melodii & Comentarii';
$LNG['reports_title'] = 'Raportări (Melodii %26 Comentarii)';
$LNG['total_tracks'] = 'Total Melodii';
$LNG['public_tracks'] = 'Melodii Publice';
$LNG['private_tracks'] = 'Melodii Private';
$LNG['total_comments'] = 'Total Comentarii';
$LNG['stats_total'] = 'Total';
$LNG['stats_public'] = 'Public';
$LNG['stats_private'] = 'Privat';
$LNG['reports'] = 'Raportări';
$LNG['total_reports'] = 'Total Raportări';
$LNG['pending_reports'] = 'Raportări în Așteptare';
$LNG['safe_reports'] = 'Raportări Sigure';
$LNG['deleted_reports'] = 'Raportări Șterse';
$LNG['likes_today'] = 'Aprecieri astăzi';
$LNG['likes_this_month'] = 'Aprecieri luna asta';
$LNG['likes_last_30'] = 'Ultimele 30 de zile';
$LNG['likes_total'] = 'Total Aprecieri';
$LNG['likes'] = 'Aprecieri';
$LNG['downloads_today'] = 'Descărcări Astăzi';
$LNG['downloads_this_month'] = 'Descărcări luna aceasta';
$LNG['downloads_last_30'] = 'Ultimele 30 de zile';
$LNG['downloads_total'] = 'Total Descărcări';
$LNG['playlists'] = 'Playlist-uri';
$LNG['playlist'] = 'Playlis';
$LNG['playlists_today'] = 'Playlist-uri astazi';
$LNG['playlists_this_month'] = 'Playlists luna asta';
$LNG['playlists_total'] = 'Total Playlist-uri';
$LNG['listened_x_times'] = 'Ascultată de %s ori';
$LNG['downloaded_x_times'] = 'Descărcată de %s ori';
$LNG['payments'] = 'Plăți';
$LNG['total_payments'] = 'Total Plăți';
$LNG['payments_today'] = 'Plăți astăzi';
$LNG['payments_this_month'] = 'Plăți luna asta';
$LNG['payments_last_30'] = 'Ultimele 30 zile';
$LNG['earnings'] = 'Câștiguri';
$LNG['earnings_today'] = 'Câștiguri astăzi';
$LNG['earnings_this_month'] = 'Câștiguri luna asta';
$LNG['earnings_last_30'] = 'Ultimele 30 de zile';
$LNG['total_earnings'] = 'Total Câștiguri';

// DASHBOARD
$LNG['admin_panel'] = 'Panou Administrator';
$LNG['at_a_glance'] = 'Dintr-o privire';
$LNG['site_info'] = 'Info Site';
$LNG['site_version'] = '<a href="%s" target="_blank">%s</a> %s cu tema <a href="%s" rel="loadpage">%s</a> activată';
$LNG['online_users'] = 'Utilizatori Online';
$LNG['useful_links'] = 'Linkuri folositoare';
$LNG['get_themes'] = 'Obține mai multe Teme';
$LNG['get_languages'] = 'Obține mai multe Limbi';

// PLAYER //
$LNG['mute'] = 'Mut';
$LNG['unmute'] = 'Activați sunetul';
$LNG['repeat'] = 'Repetă';
$LNG['repeat_off'] = 'Nu mai repeta';
$LNG['play'] = 'Redă';
$LNG['pause'] = 'Pauză';
$LNG['prev_track'] = 'Melodia anterioară';
$LNG['next_track'] = 'Următoarea melodie';
$LNG['volume'] = 'Volum';

// SHARE //
$LNG['share_facebook'] = 'Împărtășește pe Facebook';
$LNG['share_twitter'] = 'Împărtășește pe Twitter';
$LNG['share_pinterest'] = 'Împărtășește pe Pinterest';
$LNG['share_email'] = 'Împărtășește pe email';
$LNG['share_sn'] = 'Împărtășește pe rețelele de socializare';
$LNG['share_url'] = 'Împărtășește URL';
$LNG['embed'] = 'Embed';
$LNG['embed_code'] = 'Cod Embed';
$LNG['embed_error'] = 'Ne pare rău, nu am putut găsi melodia cerută.';

// LIKES //
$LNG['already_liked'] = 'Ai apreciat deja această melodie.';
$LNG['already_disliked'] = 'Ai dezapreciat deja această melodie.';
$LNG['like'] = 'Apreciază';
$LNG['dislike'] = 'Apreciată';
$LNG['like_track_not_exist'] = 'Această melodie nu există sau a fost ștearsă.';
$LNG['liked_this'] = 'a apreciat';

// MISC //
$LNG['from'] = 'De la';
$LNG['to'] = 'Pană la';
$LNG['type'] = 'Tip';
$LNG['status'] = 'Stare';
$LNG['sponsored'] = 'Sponsorizat';
$LNG['new_like_notification'] = '<a href="%s" rel="loadpage">%s</a> a apreciat <a href="%s" rel="loadpage">melodia</a> ta';
$LNG['new_comment_notification'] = '<a href="%s" rel="loadpage">%s</a> a comentat <a href="%s" rel="loadpage">melodia</a> ta';
$LNG['new_friend_notification'] = '<a href="%s" rel="loadpage">%s</a> te-a adăugat ca prieten';
$LNG['new_chat_notification'] = '<a href="%s" rel="loadpage">%s</a> ți-a trimis un <a href="%s" rel="loadpage">mesaj în chat</a>';
$LNG['new_like_fa'] = '<strong><a href="%s" rel="loadpage">%s</a></strong> ți-a apreciat <strong><a href="%s" rel="loadpage">melodia</a></strong>';
$LNG['new_comment_fa'] = '<strong><a href="%s" rel="loadpage">%s</a></strong> ți-a comentat <strong><a href="%s" rel="loadpage">melodia</a></strong>';
$LNG['new_track_fa'] = '<strong><a href="%s" rel="loadpage">%s</a></strong> a încărcat o <strong><a href="%s" rel="loadpage">melodie</a> nouă</strong>';
$LNG['change_password'] = 'Schimbă Parola';
$LNG['enter_new_password'] = 'Introdu noua parolă';
$LNG['enter_reset_key'] = 'Introu cheia de resetare';
$LNG['enter_username'] = 'Introdu Utilizator';
$LNG['remember_me'] = 'Amintește-mă';
$LNG['reset_key'] = 'Cheie de resetare';
$LNG['new_password'] = 'Parolă nouă';
$LNG['password_recovery'] = 'Recuperare parolă';
$LNG['recover']	= 'Recuperează';
$LNG['delete'] = 'Șterge';
$LNG['suspend'] = 'Suspendă';
$LNG['restore'] = 'Restaurează';
$LNG['enable'] = 'Activează';
$LNG['general'] = 'General';
$LNG['registration'] = 'Înregistrare';
$LNG['limits'] = 'Limitări';
$LNG['emails'] = 'Emailuri';
$LNG['storage'] = 'Stocare';
$LNG['recover_sub_username'] = 'Introdu numele de utilizator pentru care dorești recuperarea parolei';
$LNG['delete_track_desc'] = 'Ești sigur(ă) că vrei să ștergi această melodie?';
$LNG['delete_playlist_desc'] = 'Esti sigur(ă) că vrei să ștergi acest playlist?';
$LNG['delete_account'] = 'Ștergerea unui cont rezultă în ștergerea tuturor informațiilor de pe server, acțiunea fiind ireversibilă.';
$LNG['suspend_account'] = 'Suspendarea unui cont va restricționa accesul utilizatorului la cont și va ascunde conținutul acestuia, contul putând fi restaurat.';
$LNG['restore_account'] = 'Restaurarea unui cont îi va restitui accesul la cont și conținut.';
$LNG['account_suspended'] = 'Acest cont este momentan suspendat.';
$LNG['account_not_activated'] = 'Acest cont nu este activat. <a href="%s">Click aici</a> pentru a retrimite emailul de activare.';
$LNG['re_activate_already'] = 'A fost deja trimis un email de activare astăzi';
$LNG['re_activate_sent'] = 'Un email de activare a fost trimis';
$LNG['view'] = 'Vezi';
$LNG['uploading'] = 'Se încarcă';
$LNG['processing'] = 'Se procesează...';
$LNG['tracking_code'] = 'Cod de monitorizare';

// PROFILE //
$LNG['profile_not_exist'] = 'Ne pare rău, dar acest profil nu există.';
$LNG['profile_semi_private'] = 'Ne pare rău, dar acest profil este privat, doar prietenii acestui utilizator îl pot vedea.';
$LNG['profile_private'] = 'Ne pare rău, dar acest profil este complet privat.';
$LNG['profile_suspended'] = 'Ne pare rău, dar acest profil este suspendat.';
$LNG['profile_not_exist_ttl'] = 'Profilul nu există';
$LNG['profile_semi_private_ttl'] = 'Profilul este privat';
$LNG['profile_private_ttl'] = 'Profilul este privat';
$LNG['profile_suspended_ttl'] = 'Profil Suspendat';
$LNG['add_friend'] = 'Urmarește';
$LNG['remove_friend'] = 'Anulează urmărirea';
$LNG['profile_about'] = 'Despre';
$LNG['profile_location'] = 'Locație';
$LNG['profile_website'] = 'Pagină';
$LNG['profile_view_site'] = 'Vezi website';
$LNG['profile_view_profile'] = 'Vezi profil';
$LNG['profile_view_social'] = 'Vezi profilul %s';
$LNG['pro_user'] = 'Cont Pro';
$LNG['edit_profile_cover'] = 'Schimbă imaginile de profil';
$LNG['view_all_notifications'] = 'Vezi mai multe notificări';
$LNG['view_chat_notifications'] = 'Vezi mai multe mesaje';
$LNG['close_notifications'] = 'Închide notificările';
$LNG['notifications_settings'] = 'Setări Notificări';
$LNG['no_notifications'] = 'Nici o notificare';
$LNG['search_title'] = 'Rezultate Căutare';
$LNG['view_all_results'] = 'Vezi toate rezultatele';
$LNG['close_results'] = 'Închide Rezultatele';
$LNG['no_results'] = 'Nici un rezultat. Încearcă o căutare nouă.';
$LNG['no_results_ttl'] = 'Rezultate Căutare';
$LNG['search_for_users'] = 'Caută prieteni';
$LNG['search_in_friends'] = 'Caută în prieteni';
$LNG['follows'] = 'Urmărește';
$LNG['followed_by'] = 'Urmărit de';
$LNG['people'] = 'persoane';
$LNG['join_date'] = 'Data înregistrării';

// PHP MODULES
$LNG['openssl_error'] = 'Trebuie să activezi extensia <strong>OpenSSL</strong> pe server';
$LNG['curl_error'] = 'Este recomandat ca extensia <strong>cURL</strong> să fie activată pe server';

// PRO MESSAGES
$LNG['go_pro_0'] = 'Obține statistici avansate cum ar fi unde este localizată audiența sau cine ți-a redat melodiile.';
$LNG['go_pro_1'] = 'Obține statistici avansate statistici despre audiența melodiilor tale.';
$LNG['go_pro_2'] = 'Obține mai mult spațiu de stocare și urcă melodii mai luni.';
$LNG['go_pro_ttl_1'] = 'Află mai multe';
$LNG['go_pro_ttl_2'] = 'Primește mai multe';
$LNG['go_pro_3'] = 'Obține statistici avansate cum ar fi unde este localizată audiența sau cine ți-a redat melodiile, mai mult spațiu de stocare și urcă mai multe melodii.';
$LNG['go_pro_4'] = 'Obține mai mult spațiu de stocare pentru melodiile tale, stai în legătură cu audiența ta, vezi statistici avansate, și mai multe...';
$LNG['current_plan'] = 'Planul tău curent este';
$LNG['plan_0'] = 'Standard';
$LNG['plan_1'] = 'Pro';
$LNG['view_pricing'] = 'Vezi Prețuri';
$LNG['total_space'] = 'Spațiu Total';
$LNG['total_space_desc'] = 'Spațiu total permis pentru încărcare.';
$LNG['track_size'] = 'Dimensiune Melodie';
$LNG['track_size_desc'] = 'Dimensiunea permisă de încărcare pe melodie.';
$LNG['basic_stats'] = 'Statistici de bază';
$LNG['basic_stats_desc'] = 'Contorizează redări, aprecieri & comentarii.';
$LNG['extended_stats'] = 'Statistici extinse';
$LNG['extended_stats_desc'] = 'Află cine îți redă melodiile și în ce țară sunt cele mai redate.';
$LNG['pro_badge'] = 'Insigă Pro';
$LNG['pro_badge_desc'] = 'Insingă Pro care apare în contul tău.';
$LNG['pro_month'] = 'Plan Pro Lunar - %s'; 
$LNG['pro_year'] = 'Plan Pro Anual - %s';
$LNG['go_pro_desc'] = 'Mai mult spațiu de încărcare, statistici, facilități';
$LNG['go_pro_congrats'] = 'Contul tău a fost upgradat la planul Pro';
$LNG['payment_error_0'] = 'Cererea a eșuat';
$LNG['payment_error_1'] = 'Plata a fost anulată.';
$LNG['pro_plan'] = 'Plan Pro';
$LNG['pro_plan_desc'] = 'Tipul de plan Pro';
$LNG['valid'] = 'Valid';
$LNG['valid_desc'] = 'Valablitatea stării contului.';
$LNG['expire_on'] = 'Expiră la';
$LNG['monthly'] = 'Lunar';
$LNG['yearly'] = 'Anual';
$LNG['days_left'] = 'zile rămase';
$LNG['account_status'] = 'Starea curentă a contului';
$LNG['transactions_history'] = 'Istoric tranzacții';
$LNG['history'] = 'Istorie';
$LNG['payments'] = 'Plăți';
$LNG['completed'] = 'Completă';
$LNG['refunded'] = 'Rambursată';
$LNG['reversed'] = 'Inversată';
$LNG['pending'] = 'În așteptare';
$LNG['suspended'] = 'Suspendată';
$LNG['failed'] = 'Eșuată';
$LNG['denied'] = 'Refuzată';
$LNG['promote'] = 'Promovează';
$LNG['promoted'] = 'Contul a fost promovat la statutul de Pro.';
$LNG['promote_info'] = 'Promovează acest cont la statutul de Pro';

// MANAGE PAYMENTS //
$LNG['amount'] = 'Valoare';
$LNG['date'] = 'Data';
$LNG['plan'] = 'Plan';
$LNG['transaction_id'] = 'ID Tranzacție';
$LNG['transaction_details'] = 'Detalii Tranzacție';

// GENERAL //
$LNG['title_profile'] = 'Profil';
$LNG['title_messages'] = 'Mesaje';
$LNG['title_settings'] = 'Setări';
$LNG['explore'] = 'Exploră';
$LNG['search'] = 'Caută';
$LNG['title_notifications'] = 'Notificări';
$LNG['title_admin']	= 'Admin';
$LNG['on'] = 'On';
$LNG['off'] = 'Off';
$LNG['no'] = 'Nu';
$LNG['yes'] = 'Da';
$LNG['none'] = 'Nici unul';
$LNG['add'] = 'Adaugă';
$LNG['save'] = 'Salvează';
$LNG['send'] = 'Trimite';
$LNG['all'] = 'Toate';
$LNG['default'] = 'Principală';
$LNG['make_default'] = 'Fă-o principală';
$LNG['categories'] = 'Categorii';
$LNG['stream'] = 'Stream';
$LNG['track'] = 'Melodie';
$LNG['tracks'] = 'Melodii';
$LNG['pages'] = 'Pagini';
$LNG['report'] = 'Raport';
$LNG['other'] = 'Alte';
$LNG['stats'] = 'Statistici';
$LNG['author'] = 'Autor';
$LNG['latest_music'] = 'Ultimele Melodii';
$LNG['popular_music'] = 'Melodii Populare';
$LNG['liked_music'] = 'Melodii Apreciate';
$LNG['go_pro'] = 'Deveniți Pro';
$LNG['edit'] = 'Editează';
$LNG['shuffle'] = 'Amestecă';
$LNG['upload'] = 'Încarcă';
$LNG['features'] = 'Facilități';
$LNG['search_for_people'] = 'Caută...';
$LNG['privacy_policy'] = 'Politică de confidențialitate';
$LNG['terms_of_use'] = 'Termeni de utilizare';
$LNG['about'] = 'Despre';
$LNG['disclaimer'] = 'Disclaimer';
$LNG['contact'] = 'Contact';
$LNG['developers'] = 'Dezvoltatori';
$LNG['language'] = 'Limbă';

// TIME
$LNG['just_now'] = 'acum';
$LNG['ta_second'] = 'o secundă';
$LNG['ta_seconds'] = '%d secunde';
$LNG['ta_minute'] = 'un minut';
$LNG['ta_minutes'] = '%d minute';
$LNG['ta_hour'] = 'o oră';
$LNG['ta_hours'] = '%d ore';
$LNG['ta_day'] = 'o zi';
$LNG['ta_days'] = '%d zile';
$LNG['ta_week'] = 'o săptămână';
$LNG['ta_weeks'] = '%d săptămâni';
$LNG['ta_month'] = 'o lună';
$LNG['ta_months'] = '%d luni';
$LNG['ta_year'] = 'un an';
$LNG['ta_years'] = '%d ani';
$LNG['ago'] = 'în urmă';

// MONTHS
$LNG['month'] = 'Luna';
$LNG['year'] = 'An';
$LNG['day'] = 'Zi';
$LNG['month_1'] = 'Ianuarie';
$LNG['month_2'] = 'Februarie';
$LNG['month_3'] = 'Martie';
$LNG['month_4'] = 'Aprile';
$LNG['month_5'] = 'Mai';
$LNG['month_6'] = 'Iunie';
$LNG['month_7'] = 'Iulie';
$LNG['month_8'] = 'August';
$LNG['month_9'] = 'Septembrie';
$LNG['month_10'] = 'Octombrie';
$LNG['month_11'] = 'Noiembrie';
$LNG['month_12'] = 'Decembrie';
?>